...
class LowpanHardwareModeCluster(
  private val controller: MatterController,
  private val endpointId: UShort,
) {
  class ChangeToModeResponse(val status: UByte, val statusText: String?)

  class SupportedModesAttribute(val value: List<LowpanHardwareModeClusterModeOptionStruct>)

  sealed class SupportedModesAttributeSubscriptionState {
    data class Success(val value: List<LowpanHardwareModeClusterModeOptionStruct>) :
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    val decodedValue: List<LowpanHardwareModeClusterModeOptionStruct> =
      buildList<LowpanHardwareModeClusterModeOptionStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(LowpanHardwareModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }
  ...
  companion object {
    private val logger = Logger.getLogger(LowpanHardwareModeCluster::class.java.name)
    const val CLUSTER_ID: UInt = 4294114309u
  }
}
