...
class LowpanBleSensorCluster(private val controller: MatterController, private val endpointId: UShort) {class SensorsAttribute(
    val value: List<LowpanBleSensorClusterSensorStruct>
  )
...
class AttributeListAttribute(
  ...
  suspend fun addSensorEp(macAddress: String
    ...
    val TAG_MAC_ADDRESS_REQ: Int = 0
    tlvWriter.put(ContextSpecificTag(TAG_MAC_ADDRESS_REQ), macAddress)    
  ...
  suspend fun removeSensorEp(endpoint: ULong
      ,timedInvokeTimeout: Duration? = null) {
...
suspend fun readSensorsAttribute(): SensorsAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Sensors attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<LowpanBleSensorClusterSensorStruct> = buildList<LowpanBleSensorClusterSensorStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(LowpanBleSensorClusterSensorStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }


    return SensorsAttribute(decodedValue)
  }

  suspend fun subscribeSensorsAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SensorsAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SensorsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Sensors attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(SensorsAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SensorsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(LowpanBleSensorCluster::class.java.name)
    const val CLUSTER_ID: UInt = 4294114307u
  }
}
