...
class LowpanGroupCluster(private val controller: MatterController, private val endpointId: UShort) {class GeneratedCommandListAttribute(
    val value: List<UInt>
  )
...
class AttributeListAttribute(
  ...
  suspend fun addGroupEp(endpoint: UShort
      ,groupName: String
  ...
  suspend fun removeGroupEp(endpoint: UShort
      ,timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun configuration(timedInvokeTimeout: Duration? = null) {
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(LowpanGroupCluster::class.java.name)
    const val CLUSTER_ID: UInt = 4294114305u
  }
}
