...
class LowpanHardwareModeClusterModeOptionStruct(
  val label: String,
  val mode: UInt,
  val modeTags: List<LowpanHardwareModeClusterModeTagStruct>,
) {
  override fun toString(): String = buildString {
    append("LowpanHardwareModeClusterModeOptionStruct {\n")
    append("\tlabel : $label\n")
    append("\tmode : $mode\n")
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): LowpanHardwareModeClusterModeOptionStruct {
      tlvReader.enterStructure(tlvTag)
      val label = tlvReader.getString(ContextSpecificTag(TAG_LABEL))
      val mode = tlvReader.getUInt(ContextSpecificTag(TAG_MODE))
      val modeTags =
        buildList<LowpanHardwareModeClusterModeTagStruct> {
          tlvReader.enterArray(ContextSpecificTag(TAG_MODE_TAGS))
          while (!tlvReader.isEndOfContainer()) {
            add(LowpanHardwareModeClusterModeTagStruct.fromTlv(AnonymousTag, tlvReader))
          }
      ...
      return LowpanHardwareModeClusterModeOptionStruct(label, mode, modeTags)
    }
  }
}
