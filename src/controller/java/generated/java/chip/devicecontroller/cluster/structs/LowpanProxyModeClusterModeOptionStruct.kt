...
class LowpanProxyModeClusterModeOptionStruct(
  val label: String,
  val mode: UInt,
  val modeTags: List<LowpanProxyModeClusterModeTagStruct>,
) {
  override fun toString(): String = buildString {
    append("LowpanProxyModeClusterModeOptionStruct {\n")
    append("\tlabel : $label\n")
    append("\tmode : $mode\n")
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): LowpanProxyModeClusterModeOptionStruct {
      tlvReader.enterStructure(tlvTag)
      val label = tlvReader.getString(ContextSpecificTag(TAG_LABEL))
      val mode = tlvReader.getUInt(ContextSpecificTag(TAG_MODE))
      val modeTags =
        buildList<LowpanProxyModeClusterModeTagStruct> {
          tlvReader.enterArray(ContextSpecificTag(TAG_MODE_TAGS))
          while (!tlvReader.isEndOfContainer()) {
            add(LowpanProxyModeClusterModeTagStruct.fromTlv(AnonymousTag, tlvReader))
          }
      ...
      return LowpanProxyModeClusterModeOptionStruct(label, mode, modeTags)
    }
  }
}
