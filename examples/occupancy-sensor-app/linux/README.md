# Matter Linux Pressure Measurement App Example

1. Build the sample app and the chip tool

    ```bash
    ./scripts/build/build_examples.py --target linux-arm64-pressure-measurement --target linux-arm64-chip-tool build
    ```

2. Launch the sample app

    ```bash
    ./out/linux-arm64-pressure-measurement/pressure-measurement-app
    ```
...
5. Update the LeakStatus to "true""
    ...
    ```bash
    echo '{"Name":"LeakStatus","NewValue":1}' > /tmp/chip_pressure_measurement_fifo_*
    ```
...
7. Update the LeakStatus to "false"
    ...
    ```bash
    echo '{"Name":"LeakStatus","NewValue":0}' > /tmp/chip_pressure_measurement_fifo*
    ```
