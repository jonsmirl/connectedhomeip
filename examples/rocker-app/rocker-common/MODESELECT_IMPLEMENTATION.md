# ModeSelect Cluster Implementation for Rocker-App

## Overview

This document describes the ModeSelect cluster implementation for EP2 (Smart Switch endpoint) in the rocker-app. The implementation provides a framework for selecting different operational modes that will control how EP2 commands are processed.
...
#### 1. RockerModeSelect.h/cpp
- **RockerModeSelectDelegate**: Implements `ModeSelect::SupportedModesManager::Delegate`
...
void emberAfModeSelectClusterInitCallback(chip::EndpointId endpointId)
{
    ...
    if (endpointId != RockerManager::kSwitchEndpointId) return;
    
    // Create delegate and supported modes manager
    gRockerModeSelectDelegate = new RockerModeSelectDelegate;
    gRockerModeSelectInstance = new ModeSelect::SupportedModesManager(endpointId, gRockerModeSelectDelegate);
...
3. **Manager Notification**: Calls `RockerManager::HandleModeChange()`
4. **Response**: Sends appropriate status response to client

### Mode Change Processing

When a mode change occurs, `RockerManager::HandleModeChange()` is called:
...
- **Attributes**: 
  - Description: "Rocker Switch Mode"
...
sources = [
  ...
  "src/RockerManager.cpp",
  "src/RockerModeSelect.cpp",
  "src/ZclCallbacks.cpp",
]
```

### ZAP Configuration
The ModeSelect cluster is configured in `rocker-app.zap` for EP2 with:
...
- [ ] RockerManager state is updated on mode changes
