...
CHIP_ERROR RockerManager::Init()
{
    ...
    ChipLogProgress(AppServer, "RockerManager: Initializing dual-endpoint device");
    ...
    ChipLogProgress(AppServer, "RockerManager: Initialization complete");
...
void RockerManager::Shutdown()
{
    ...
    ChipLogProgress(AppServer, "RockerManager: Shutting down");
    mInitialized = false;
}

void RockerManager::SetLightOnOff(bool isOn)
{
    if (mActualLightState.isOn != isOn)
    {
        // Update the centralized actual light state
        mActualLightState.isOn = isOn;
        ChipLogProgress(AppServer, "RockerManager: Actual Light OnOff changed to %s", isOn ? "ON" : "OFF");
        ...
        DeviceLayer::PlatformMgr().ScheduleWork(
            [](intptr_t context) {
                ...
                {
                    ChipLogError(AppServer, "RockerManager: Failed to sync EP1 OnOff attribute: %x", to_underlying(status));
                }
                else
                {
                    ChipLogProgress(AppServer, "RockerManager: EP1 OnOff attribute synchronized to %s", newValue ? "ON" : "OFF");
...
void RockerManager::SetLightLevel(uint8_t level)
{
    if (mActualLightState.level != level)
    {
        // Update the centralized actual light state
        mActualLightState.level = level;
        ChipLogProgress(AppServer, "RockerManager: Actual Light Level changed to %d", level);
        ...
        DeviceLayer::PlatformMgr().ScheduleWork(
            [](intptr_t context) {
                ...
                if (status != Protocols::InteractionModel::Status::Success)
                {
                    ChipLogError(AppServer, "RockerManager: Failed to sync EP1 CurrentLevel attribute: %x", to_underlying(status));
                }
                else
                {
                    ChipLogProgress(AppServer, "RockerManager: EP1 CurrentLevel attribute synchronized to %d", newLevel);
...
void RockerManager::SetLightColor(uint8_t hue, uint8_t saturation)
{
    ...
    if (changed)
    {
        ChipLogProgress(AppServer, "RockerManager: Actual Light Color changed to ColorXY X:0x%04X Y:0x%04X (from H:%d S:%d)",
        ...
        DeviceLayer::PlatformMgr().ScheduleWork(
            [](intptr_t context) {
                ...
                if (status == Protocols::InteractionModel::Status::Success)
                {
                    ChipLogProgress(AppServer, "RockerManager: EP1 ColorMode set to ColorXY");
                ...
                if (status == Protocols::InteractionModel::Status::Success)
                {
                    ChipLogProgress(AppServer, "RockerManager: EP1 CurrentX synchronized to 0x%04X", colorX);
                    GetInstance().TriggerAttributeChangeNotification(kLightEndpointId, ColorControl::Id,
                ...
                if (status == Protocols::InteractionModel::Status::Success)
                {
                    ChipLogProgress(AppServer, "RockerManager: EP1 CurrentY synchronized to 0x%04X", colorY);
                    GetInstance().TriggerAttributeChangeNotification(kLightEndpointId, ColorControl::Id,
...
void RockerManager::UpdateActualLightSaturation(uint8_t saturation)
{
...
void RockerManager::UpdateActualLightColorXY(uint16_t colorX, uint16_t colorY)
{
    ...
    ChipLogProgress(AppServer, "RockerManager: Actual Light ColorXY changed to X:0x%04X Y:0x%04X", colorX, colorY);

    // Schedule EP1 synchronization on the Matter main loop
    DeviceLayer::PlatformMgr().ScheduleWork(
        [](intptr_t context) { RockerManager::GetInstance().SynchronizeEP1ColorXYAttributes(); });
}

CHIP_ERROR RockerManager::SendOnOffCommand(bool isOn)
{
    ChipLogProgress(AppServer, "RockerManager: Sending OnOff command: %s", isOn ? "ON" : "OFF");
    ...
    ChipLogProgress(AppServer, "RockerManager: Local OnOff command completed");
    return CHIP_NO_ERROR;
}

CHIP_ERROR RockerManager::SendLevelCommand(uint8_t level)
{
    ChipLogProgress(AppServer, "RockerManager: Sending Level command: %d", level);

    // Implement local device control: directly update the light endpoint
    // This simulates the switch endpoint controlling the light endpoint within the same device
    SetLightLevel(level);

    ChipLogProgress(AppServer, "RockerManager: Local Level command completed");
    return CHIP_NO_ERROR;
}

CHIP_ERROR RockerManager::SendColorCommand(uint8_t hue, uint8_t saturation)
{
    ChipLogProgress(AppServer, "RockerManager: Sending Color command: H:%d S:%d", hue, saturation);

    // Implement local device control: directly update the light endpoint
    // This simulates the switch endpoint controlling the light endpoint within the same device
    SetLightColor(hue, saturation);

    ChipLogProgress(AppServer, "RockerManager: Local Color command completed");
    return CHIP_NO_ERROR;
}

CHIP_ERROR RockerManager::SendColorXYCommand(uint16_t colorX, uint16_t colorY)
{
    ChipLogProgress(AppServer, "RockerManager: Sending ColorXY command: X:0x%04X Y:0x%04X", colorX, colorY);
    ...
    ChipLogProgress(AppServer, "RockerManager: Local ColorXY command completed");
    return CHIP_NO_ERROR;
}

CHIP_ERROR RockerManager::SendRGBCommand(uint8_t red, uint8_t green, uint8_t blue)
{
    ChipLogProgress(AppServer, "RockerManager: Sending RGB command: R:%d G:%d B:%d", red, green, blue);
    ...
    ChipLogProgress(AppServer, "RockerManager: RGB command set ColorXY:X:0x%04X Y:0x%04X (Level preserved: %d)", colorX, colorY,
                    currentLevel);
    ChipLogProgress(AppServer, "RockerManager: Local RGB command completed");
    return CHIP_NO_ERROR;
}

CHIP_ERROR RockerManager::SendIdentifyCommand()
{
    ChipLogProgress(AppServer, "RockerManager: Sending Identify command to EP%d", kLightEndpointId);
    ...
    ChipLogProgress(AppServer, "RockerManager: Identify command - would trigger %d seconds identify on EP%d", kIdentifyTimeSeconds,
                    kLightEndpointId);
    ChipLogProgress(AppServer, "RockerManager: Note: For external devices, use binding cluster to configure targets");

    return CHIP_NO_ERROR;
}

void RockerManager::TriggerAttributeChangeNotification(EndpointId endpointId, ClusterId clusterId, AttributeId attributeId)
{
    ...
    ChipLogProgress(AppServer,
                    "RockerManager: Triggered attribute change notification for Endpoint=%d, Cluster=0x%08lX, Attribute=0x%08lX",
                    endpointId, static_cast<unsigned long>(clusterId), static_cast<unsigned long>(attributeId));
}

void RockerManager::UpdateActualLightStateAndSyncEP1(bool isOn, uint8_t level, uint8_t hue, uint8_t saturation)
{
    ...
    {
        ChipLogProgress(AppServer,
                        "RockerManager: Actual Light State updated - OnOff:%s Level:%d ColorXY X:0x%04X Y:0x%04X (from H:%d S:%d)",
...
void RockerManager::SynchronizeEP1WithActualState()
{
    ChipLogProgress(AppServer, "RockerManager: Synchronizing EP1 attributes with actual light state");

    // Schedule all EP1 attribute updates on the Matter main loop thread for thread safety
    DeviceLayer::PlatformMgr().ScheduleWork(
        [](intptr_t context) {
            RockerManager & manager        = GetInstance();
            ...
            if (status == Protocols::InteractionModel::Status::Success)
            {
                ChipLogProgress(AppServer, "RockerManager: EP1 OnOff synchronized to %s", state.isOn ? "ON" : "OFF");
            ...
            if (status == Protocols::InteractionModel::Status::Success)
            {
                ChipLogProgress(AppServer, "RockerManager: EP1 Level synchronized to %d", state.level);
            ...
            if (status == Protocols::InteractionModel::Status::Success)
            {
                ChipLogProgress(AppServer, "RockerManager: EP1 ColorMode synchronized to %d", state.colorMode);
            ...
            if (status == Protocols::InteractionModel::Status::Success)
            {
                ChipLogProgress(AppServer, "RockerManager: EP1 CurrentX synchronized to 0x%04X", state.colorX);
                manager.TriggerAttributeChangeNotification(kLightEndpointId, ColorControl::Id,
                                                           ColorControl::Attributes::CurrentX::Id);
            }
            ...
            if (status == Protocols::InteractionModel::Status::Success)
            {
                ChipLogProgress(AppServer, "RockerManager: EP1 CurrentY synchronized to 0x%04X", state.colorY);
                manager.TriggerAttributeChangeNotification(kLightEndpointId, ColorControl::Id,
                                                           ColorControl::Attributes::CurrentY::Id);
            }
        },
...
void RockerManager::SynchronizeEP1ColorXYAttributes()
{
    ChipLogProgress(AppServer, "RockerManager: Synchronizing EP1 ColorXY attributes with actual light state");
...
void RockerManager::HueSaturationToColorXY(uint8_t hue, uint8_t saturation, uint16_t & colorX, uint16_t & colorY)
{
...
void RockerManager::ColorXYToHueSaturation(uint16_t colorX, uint16_t colorY, uint8_t & hue, uint8_t & saturation)
{
...
void RockerManager::RGBToColorXY(uint8_t red, uint8_t green, uint8_t blue, uint16_t & colorX, uint16_t & colorY)
{
...
void RockerManager::ColorXYToRGB(uint16_t colorX, uint16_t colorY, uint8_t & red, uint8_t & green, uint8_t & blue)
{
...
void RockerManager::SetCurrentMode(uint8_t mode)
{
    if (mCurrentMode != mode)
    {
        mCurrentMode = mode;
        ChipLogProgress(AppServer, "RockerManager: Current mode changed to %d", mode);
    }
}

void RockerManager::HandleModeChange(uint8_t newMode)
{
    ChipLogProgress(AppServer, "RockerManager: Handling mode change to %d", newMode);
    ...
    ChipLogProgress(AppServer, "RockerManager: Mode changed to %d (%s)", newMode, modeName);
