# CHIP Linux Rocker App Example

The Rocker App is a dual-endpoint Matter device that combines the functionality of both a lighting device and a light switch device in a single application. This demonstrates how to implement a multi-endpoint device using the Matter/CHIP framework.

## Device Architecture

The Rocker device implements two distinct endpoints:
...
The "Rocker" concept represents a physical device that can both:
...
# Standard build
gn gen out/debug --args='import("//examples/rocker-app/linux/args.gni")'
ninja -C out/debug chip-rocker-app

# Build with PW RPC support
gn gen out/debug --args='import("//examples/rocker-app/linux/with_pw_rpc.gni")'
ninja -C out/debug chip-rocker-app
```

### Build Arguments

The rocker app supports the following build arguments in `args.gni`:
...
```bash
# Run the rocker app
./out/debug/chip-rocker-app

# Run with custom parameters
./out/debug/chip-rocker-app --secured-device-port 5540 --discriminator 3840
```

### Command Line Options

The rocker app supports standard CHIP application options:
...
The rocker device can be commissioned like any other Matter device:
...
# Where:
# - 1 = rocker device node ID
# - 2 = rocker switch endpoint
...
- ✅ RockerManager for state management
...
- `rocker-app.zap` - Main device configuration with both endpoints
...
└── rocker-common/
    ...
    ├── include/
    │   └── RockerManager.h    # Dual-endpoint manager
    └── src/
        └── RockerManager.cpp  # Implementation
```
