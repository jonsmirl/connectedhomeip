...
OptionDef sDeviceOptionDefs[] = {
    ...
    { "node-label", kArgumentRequired, kDeviceOption_NodeLabel },
...
const char * sDeviceOptionHelp =
    ...
    "\n"
    "  --node-label <label>\n"
    "       The user-assigned device name (max 32 characters). Sets the NodeLabel attribute in BasicInformation cluster.\n"
    "\n"
...
bool HandleOption(const char * aProgram, OptionSet * aOptions, int aIdentifier, const char * aName, const char * aValue)
{
    ...
    case kDeviceOption_NodeLabel:
        LinuxDeviceOptions::GetInstance().nodeLabel.SetValue(std::string{ aValue });
        break;
