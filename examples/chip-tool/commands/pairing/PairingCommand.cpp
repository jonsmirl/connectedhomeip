...
CHIP_ERROR PairingCommand::PairWithMdns(NodeId remoteId)
{
    ...
    // If a discriminator was provided on the base 'onnetwork' command as a flag, prefer that (long discriminator).
    if (mOnNetworkDiscriminator.HasValue())
    {
        filter.type = Dnssd::DiscoveryFilterType::kLongDiscriminator;
        filter.code = mOnNetworkDiscriminator.Value();
    }
    // Backward-compat: if a positional discriminator was provided (e.g. for BLE/NFC patterns), allow it too.
    else if (mDiscriminator.has_value())
    {
        filter.type = Dnssd::DiscoveryFilterType::kLongDiscriminator;
        filter.code = mDiscriminator.value();
    }

    // Build the filter based on the (possibly overridden) filter.type
    switch (filter.type)
    {
    ...
    case Dnssd::DiscoveryFilterType::kDeviceType:
        filter.code = (filter.code == 0 && mDiscoveryFilterCode != 0) ? mDiscoveryFilterCode : filter.code;
        break;
