...
// Custom operational credentials issuer that uses Supabase ICA to sign device certificates locally
// This mimics the behavior of the M2 Android app's certificate generation
class SupabaseOperationalCredentialsIssuer : public chip::Controller::OperationalCredentialsDelegate
{
public:
    SupabaseOperationalCredentialsIssuer();
    virtual ~SupabaseOperationalCredentialsIssuer();

    CHIP_ERROR Initialize(chip::PersistentStorageDelegate & storage);
    void SetSupabaseCertificates(const chip::ByteSpan & rcaCert, const chip::ByteSpan & icaCert);
    // Provide ICA private key generated during CSR so signatures match the ICA cert from Supabase
...
private:
    ...
    // Optional external ICA keypair reference provided by Supabase client (preferred)
