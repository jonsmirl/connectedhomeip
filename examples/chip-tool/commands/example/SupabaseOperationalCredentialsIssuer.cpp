...
SupabaseOperationalCredentialsIssuer::SupabaseOperationalCredentialsIssuer()
{
...
CHIP_ERROR SupabaseOperationalCredentialsIssuer::Initialize(chip::PersistentStorageDelegate & storage)
{
    // Initialize the ICA keypair
    ReturnErrorOnFailure(mICAKeypair.Initialize(ECPKeyTarget::ECDSA));

    ChipLogProgress(chipTool, "Supabase operational credentials issuer initialized");
    return CHIP_NO_ERROR;
}

void SupabaseOperationalCredentialsIssuer::SetSupabaseCertificates(const chip::ByteSpan & rcaCert, const chip::ByteSpan & icaCert)
{
    ...
    ChipLogProgress(chipTool, "Supabase certificates set: RCA=%zu bytes, ICA=%zu bytes", mRootCACert.size(), mICACert.size());
}

void SupabaseOperationalCredentialsIssuer::SetICAKeypairReference(chip::Crypto::P256Keypair * keypair)
{
    ...
    if (keypair != nullptr)
    {
        ...
        ChipLogProgress(chipTool, "ICA keypair reference set from Supabase client");
    }
}

CHIP_ERROR SupabaseOperationalCredentialsIssuer::ExtractICAPrivateKey()
{
    if (mICAKeyExtracted)
    {
        return CHIP_NO_ERROR;
    ...
    // Prefer an external ICA keypair that was used to generate the CSR for Supabase ICA
    if (mExternalICAKeypair != nullptr)
    {
        mICAKeyExtracted = true;
        ChipLogProgress(chipTool, "ICA private key reference available (from Supabase client)");
        return CHIP_NO_ERROR;
    }

    // Do not generate a random ICA keypair here: it would not match the Supabase ICA cert
    ChipLogError(chipTool, "ICA private key not set. Call SetICAKeypairReference() before generating NOCs");
    return CHIP_ERROR_INCORRECT_STATE;
}

CHIP_ERROR SupabaseOperationalCredentialsIssuer::CreateMatterSubjectName(chip::NodeId nodeId, chip::FabricId fabricId,
...
CHIP_ERROR SupabaseOperationalCredentialsIssuer::GenerateDeviceNOC(const chip::Crypto::P256PublicKey & devicePublicKey,
                                                                   chip::MutableByteSpan & noc)
{
    ...
    // Use the ICA from Supabase as issuer and the exact ICA private key used for the CSR
    ChipDN issuerDN;
    ...
    // Sign with the external ICA keypair (must match Supabase ICA cert)
...
CHIP_ERROR SupabaseOperationalCredentialsIssuer::GenerateNOCChain(
...
void SupabaseOperationalCredentialsIssuer::SetNodeIdForNextNOCRequest(chip::NodeId nodeId)
{
...
void SupabaseOperationalCredentialsIssuer::SetFabricIdForNextNOCRequest(chip::FabricId fabricId)
{
...
void SupabaseOperationalCredentialsIssuer::SetCATValuesForNextNOCRequest(chip::CATValues cats)
{
    mNextRequestedCATs = cats;
    ChipLogProgress(chipTool, "Set next NOC request CAT values");
}

CHIP_ERROR SupabaseOperationalCredentialsIssuer::GenerateControllerNOCChain(
    ...
    chip::MutableByteSpan & rcac, chip::MutableByteSpan & icac, chip::MutableByteSpan & noc)
{
    ...
    // Use the Supabase-provided RCAC/ICAC directly; do not generate new ones
    ...
    // Sign controller NOC with the exact ICA keypair that Supabase signed
    ...
    ChipLogProgress(chipTool, "Compressed Fabric ID (from Supabase RCAC + NOC) = 0x%016" PRIx64,
                    static_cast<uint64_t>(compressedId));

    ChipLogProgress(chipTool, "Generated controller NOC chain using Supabase RCAC/ICAC: RCA=%zu, ICA=%zu, NOC=%zu bytes",
