...
constexpr char kNodeLabelKey[]       = "pressure-measurement-app-node-label";
constexpr size_t kMaxNodeLabelLength = 32; // Matter specification limit

CHIP_ERROR PressureMeasurementAppBasicInformationAttrAccess::Read(const ConcreteReadAttributePath & aPath,
                                                                  AttributeValueEncoder & aEncoder)
{
    ...
    case Attributes::VendorID::Id: {
        // Delegate to the default BasicInformation cluster implementation
        auto * deviceInfoProvider = GetDeviceInstanceInfoProvider();
        uint16_t vendorId         = 0;
        CHIP_ERROR status         = deviceInfoProvider->GetVendorId(vendorId);
    ...
    case Attributes::ProductID::Id: {
        ...
        CHIP_ERROR status         = deviceInfoProvider->GetProductId(productId);
        if (status == CHIP_NO_ERROR)
        {
    ...
    case Attributes::ProductName::Id:
        return aEncoder.Encode(CharSpan::fromCharString("Pressure Sensor"));
    case Attributes::ProductLabel::Id:
        return aEncoder.Encode(CharSpan::fromCharString("Pressure Sensor"));
    case Attributes::PartNumber::Id:
        return aEncoder.Encode(CharSpan::fromCharString("PRESS-001"));
    case Attributes::SerialNumber::Id:
        return aEncoder.Encode(CharSpan::fromCharString("PRESS-123456"));
    case Attributes::UniqueID::Id:
        return aEncoder.Encode(CharSpan::fromCharString("PRESS-UID-123456"));
    default:
        // We don't handle this attribute, let the default implementation handle it
        return CHIP_ERROR_UNSUPPORTED_CHIP_FEATURE;
    }
}

CHIP_ERROR PressureMeasurementAppBasicInformationAttrAccess::Write(const ConcreteDataAttributePath & aPath,
...
CHIP_ERROR PressureMeasurementAppBasicInformationAttrAccess::ReadNodeLabel(AttributeValueEncoder & aEncoder)
{
...
CHIP_ERROR PressureMeasurementAppBasicInformationAttrAccess::WriteNodeLabel(AttributeValueDecoder & aDecoder)
{
    CharSpan nodeLabel;
